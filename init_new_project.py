...
def collect_git_info(args) -> Dict[str, str]:
    ...
    git_org = prompt_with_default(
        "GitHub/Azure DevOps organization",
        ""
    )
    
    git_repo = prompt_with_default(
        "Repository name",
        "fabric-cicd"
    )
...
def generate_project_config(project_info: Dict, contacts: Dict, git_info: Dict) -> Dict:
    ...
    config["metadata"]["_instructions"] = "This file was generated from project.config.template.json - DO NOT commit to version control"
...
def generate_env_file(project_info: Dict, contacts: Dict, git_info: Dict):
    ...
    if env_path.exists():
        response = input(f"{Colors.YELLOW}.env file already exists. Overwrite? (y/N): {Colors.ENDC}").strip().lower()
        if response != 'y':
...
def save_config(config: Dict):
    ...
    if config_path.exists():
        response = input(f"{Colors.YELLOW}project.config.json already exists. Overwrite? (y/N): {Colors.ENDC}").strip().lower()
        if response != 'y':
...
def main():
    ...
    print("This wizard will help you configure the Fabric CI/CD framework for your organization.")
    ...
    if not args.non_interactive:
        response = input(f"{Colors.CYAN}Ready to begin? (Y/n): {Colors.ENDC}").strip().lower()
        if response == 'n':
