...
class ProjectInitializer:
    ...
    def run(self):
        ...
        try:
            ...
            config = self._create_configuration(project_info, azure_config, github_config, contacts)
    ...
    def _collect_project_info(self) -> Dict[str, Any]:
        ...
        name = self._prompt("Project name", "fabric-cicd", 
                           "A descriptive name for your project")
        
        prefix = self._prompt("Project prefix", "your-project",
                             "Short prefix for resource naming (lowercase, hyphens allowed)")
        
        # Validate prefix
        if not re.match(r'^[a-z][a-z0-9-]*[a-z0-9]$', prefix):
            raise ValueError("Project prefix must start and end with alphanumeric, contain only lowercase letters, numbers, and hyphens")
        
        description = self._prompt("Project description", 
                                 "Microsoft Fabric CI/CD Implementation",
                                 "Brief description of the project")
        
        organization = self._prompt("Organization name", "your-org",
    ...
    def _collect_azure_config(self) -> Dict[str, Any]:
        ...
        tenant_id = self._prompt("Azure Tenant ID", "${AZURE_TENANT_ID}",
                               "Your Azure AD tenant ID")
        
        subscription_id = self._prompt("Azure Subscription ID", "${AZURE_SUBSCRIPTION_ID}",
                                     "Target Azure subscription ID")
        
        client_id = self._prompt("Azure Client ID", "${AZURE_CLIENT_ID}",
    ...
    def _collect_github_config(self) -> Dict[str, Any]:
        ...
        organization = self._prompt("GitHub Organization", "${GITHUB_ORG}",
                                  "Your GitHub organization name")
        
        repository = self._prompt("Repository name", "${GITHUB_REPO}",
    ...
    def _collect_contacts(self) -> Dict[str, Any]:
        ...
        data_owner = self._prompt("Data Owner Email", "${DATA_OWNER_EMAIL}",
                                "Primary data owner contact")
        
        technical_lead = self._prompt("Technical Lead Email", "${TECHNICAL_LEAD_EMAIL}",
                                    "Technical lead contact")
        
        devops_lead = self._prompt("DevOps Lead Email", "${DEVOPS_LEAD_EMAIL}",
                                 "DevOps lead contact")
    ...
    def _create_configuration(self, project_info: Dict[str, Any], 
                            azure_config: Dict[str, Any],
                            github_config: Dict[str, Any],
                            contacts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _update_environment_files(self, config: Dict[str, Any]):
        ...
        try:
            ...
            for env in self.environments:
                ...
                if env_file.exists():
                    print(f"📄 Environment file {env_file} will be updated automatically")
                    
        except ImportError:
            print("⚠️  ConfigManager not available - environment files need manual update")
