...
- `git_integration.py`: Feature workspace naming logic — `get_workspace_name_from_branch()` with configurable ASCII prefix (`feature_prefix`, default `[F]`) for visual identification in Fabric portal
...
- `feature_workspace.json` - Feature workspace recipe, lifecycle policies, and `feature_prefix` setting (default `[F]`)
