...
class RicohWorkspaceSetup:
        ...
        try:
            ...
            if "already exists" in str(e):
                ...
                if existing_workspace:
                    ...
                    self.setup_log["workspace_id"] = self.workspace_id
                    self.setup_log["errors"].append(f"Workspace existed (reused): {self.workspace_id}")
                    
                    return True
                else:
                    print_error("Failed to retrieve existing workspace")
                    self.setup_log["errors"].append(f"Workspace retrieval failed: {str(e)}")
                    return False
            else:
    ...
    def step_6_add_users(self):
        ...
        print_warning("User addition is commented out. Update emails and uncomment code to add users.")
        ...
        print_info("  python3 ops/scripts/manage_workspaces.py add-user --workspace leit-ricoh --email <email> --role <role>")
    ...
    def generate_summary(self):
        ...
        storage_items = [i for i in self.created_items if i.type in [FabricItemType.LAKEHOUSE, FabricItemType.WAREHOUSE]]
        notebook_items = [i for i in self.created_items if i.type == FabricItemType.NOTEBOOK]
        other_items = [i for i in self.created_items if i.type not in [FabricItemType.LAKEHOUSE, FabricItemType.WAREHOUSE, FabricItemType.NOTEBOOK]]
