...
class DomainWorkspaceSetup:
    ...
    def step_1_create_domain_workspace(self):
        ...
        try:
            ...
            if self.capacity_id:
                ...
                detail_response = self.fabric_client._make_request(
                ...
                if assigned_capacity == self.capacity_id:
                    ...
                    print_warning(
                        f"⚠️  Capacity mismatch. Expected: {self.capacity_id}, Got: {assigned_capacity}"
                    )
    ...
    def step_2_create_primary_lakehouse(self):
        ...
        domain_camel = "".join(
            word.capitalize() for word in self.domain_name.split("-")
        )
    ...
    def step_5_reference_existing_items(self):
        ...
        print_info(
            "Note: Fabric doesn't 'attach' existing items to multiple workspaces."
        )
        print_info(
            "Items belong to one workspace. To access data from other workspaces:"
        )
        ...
        if self.existing_lakehouse_workspace and self.existing_lakehouse_name:
            ...
            print_info(
                f"   4. Navigate to workspace '{self.existing_lakehouse_workspace}'"
            )
        ...
        if self.existing_warehouse_workspace and self.existing_warehouse_name:
            ...
            print_info(
                "   2. Format: [workspace_name].[warehouse_name].[schema].[table]"
            )
            print_info(
                f"   3. Example: SELECT * FROM [{self.existing_warehouse_workspace}].[{self.existing_warehouse_name}].[dbo].[Sales]"
            )
        ...
        if (
            not self.existing_lakehouse_workspace
            and not self.existing_warehouse_workspace
        ):
            print_info("\nℹ️  No existing items specified to reference.")
            print_info(
                "   Use --existing-lakehouse-workspace and --existing-lakehouse-name"
            )
    ...
    def step_6_configure_principals(self):
        ...
        if not principals_file or not principals_file.exists():
            ...
            if template_path.exists():
                ...
                with open(domain_principals_path, "w") as f:
                    ...
                    f.write(
                        '# Group:  az ad group show --group "Team Name" --query id -o tsv\n\n'
                    )
            ...
            if self.skip_user_prompt:
                ...
                print_info(
                    f"  python ops/scripts/manage_workspaces.py add-users-from-file {self.workspace_id} config/{domain_principals_path.name}"
                )
            ...
            response = (
                input(
                    "\n✏️  Press ENTER after you've edited the file (or 's' to skip): "
                )
                .strip()
                .lower()
            )
        ...
        if not has_valid_users:
            ...
            print_info(
                "The template was created but you need to add actual user/group Object IDs"
            )
            self.setup_log["principals_configured"] = False
            return True
    ...
    def step_7_generate_summary(self):
        ...
        if not self.setup_log.get("principals_configured"):
            ...
            print(
                f"     1. Edit: config/{principals_path.name if principals_path.name else f'{self.domain_name}_principals.txt'}"
            )
            print("     2. Add principals (Object IDs from Azure AD)")
            print(
                "     3. Run: python ops/scripts/manage_workspaces.py add-users-from-file \\"
            )
            print(f"             {self.workspace_id} \\")
            print(
                f"             config/{principals_path.name if principals_path.name else f'{self.domain_name}_principals.txt'}"
            )
            print("")
...
def main():
    ...
    parser.add_argument(
        "--existing-lakehouse-name", help="Name of existing lakehouse to reference"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--existing-warehouse-name", help="Name of existing warehouse to reference"
    )
    parser.add_argument(
        "--skip-user-prompt",
        action="store_true",
