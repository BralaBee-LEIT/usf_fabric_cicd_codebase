...
class ConfigDrivenWorkspace:
    ...
    def __init__(
        ...
        self.workspace_name = self.config_manager.generate_name(
            "workspace", self.environment, name=self.project_name
        )
        ...
        self.log_file = (
            self.log_dir / f"{self.project_name}_{self.environment}_setup_log.json"
        )
        ...
        print(
            f"   Naming Pattern:  {self.config_manager.get_naming_pattern('workspace')}"
        )
        print(f"\n{'='*70}\n")
        ...
        try:
            ...
            if "already exists" in str(e):
                ...
                if existing_workspace:
                    ...
                    print(f"  Workspace ID: {self.workspace_id}")
                    print(
                        f"  Display Name: {existing_workspace.get('displayName', 'N/A')}"
                    )
    ...
    def step_3_configure_principals(self):
        ...
        if self.principals_file:
            ...
            principals_filename = (
                f"{self.project_name}_{self.environment}_principals.txt"
            )
            ...
            if principals_file.exists():
                ...
                if not self.skip_user_prompt:
                    ...
                    print("   " + "─" * 60)
                    response = (
                        input("   👉 Press ENTER after editing (or 's' to skip): ")
                        .strip()
                        .lower()
                    )
                    print("   " + "─" * 60 + "\n")
                    ...
                    print(
                        "   If file has valid users, they will be added automatically\n"
                    )
        ...
        try:
            ...
            if not lines:
                ...
                print(
                    f"   python ops/scripts/manage_workspaces.py add-users-from-file {self.workspace_id} {principals_file} --yes\n"
                )
                return
            ...
            if not has_valid_format:
                print(
                    "⚠️  No valid user entries found (lines don't match expected format)"
                )
    ...
    def step_4_save_log(self):
        ...
        log_data = {
            ...
            "principals_file": (
                str(self.principals_file) if self.principals_file else None
            ),
        }
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"   URL: https://app.fabric.microsoft.com/groups/{self.workspace_id}\n"
            )
