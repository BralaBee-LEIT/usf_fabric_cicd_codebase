...
from utilities.fabric_item_manager import (
    FabricItemManager,
    FabricItemType,
    ItemDefinition,
    ItemDefinitionPart,
...
class RicohFreshWorkspaceSetup:
    ...
    def detect_available_capacity(self):
        ...
        try:
            ...
            for cap in capacities:
                print_info(
                    f"  - {cap.get('displayName')} (SKU: {cap.get('sku')}, State: {cap.get('state')})"
                )

            # Prefer trial capacity if available
            for cap in capacities:
                if "Trial" in cap.get("displayName", "") or cap.get(
                    "sku", ""
                ).startswith("FTL"):
                    print_success(
                        f"✓ Selected trial capacity: {cap.get('displayName')}"
                    )
                    return cap.get("id")
    ...
    def step_1_create_workspace_with_capacity(self):
        ...
        try:
            ...
            if not self.capacity_id:
                ...
                if not self.capacity_id:
                    print_warning(
                        "⚠️  No capacity detected - workspace will be created without capacity"
                    )
            ...
            if self.capacity_id:
                ...
                try:
                    ...
                    if assigned_capacity:
                        ...
                        print_warning(
                            "⚠️  Capacity assignment pending or not reflected in API yet"
                        )
    ...
    def create_report_definition(self, report_name: str) -> ItemDefinition:
        ...
        pbir_content = {"version": "4.0", "dataModelSchema": {"name": report_name}}
    ...
    def step_5_create_additional_items(self):
        ...
        for item_config in additional_items:
            try:
                ...
                if item_config.get("definition") == "semantic_model":
                    ...
                    definition = self.create_semantic_model_definition(
                        item_config["name"]
                    )
        ...
        print_info(
            "These items require complex definitions and are best created through:"
        )
    ...
    def step_6_configure_users(self):
        ...
        if self.principals_file:
            ...
            workspace_file = (
                ...
                / f"{self.workspace_name.split('-')[0]}_principals.txt"
            )
        ...
        if not principals_file or not principals_file.exists():
            ...
            print_info(
                f"     python ops/scripts/manage_workspaces.py add-users-from-file {self.workspace_id} my_workspace_users.txt"
            )
            print_info("\n  Option 2 - Add individual user/group:")
            print_info(
                f"     python ops/scripts/manage_workspaces.py add-user {self.workspace_id} <object_id> --role Admin --principal-type User"
            )
            print_info("\n  Get Object IDs:")
            print_info(
        ...
        try:
            ...
            if add_result.returncode != 0:
                ...
                self.setup_log["errors"].append(
                    f"User configuration failed: {add_result.stderr}"
                )
                return False
    ...
    def run(self):
        ...
        steps = [
            (
                "Create Workspace with Capacity",
                self.step_1_create_workspace_with_capacity,
            ),
        ...
        print(
            f"     python ops/scripts/manage_workspaces.py add-users-from-file {self.workspace_id} <principals_file>"
        )
...
def main():
    ...
    setup = RicohFreshWorkspaceSetup(
        capacity_id=args.capacity_id, principals_file=args.principals_file
    )
    exit_code = setup.run()
    sys.exit(exit_code)
