...
class AuditLogger:
    ...
    def __init__(
        self, audit_file: Optional[Path] = None, auto_create_dirs: bool = True
    ):
        """
    ...
    def log_workspace_update(self, workspace_id: str, changes: Dict[str, Any]) -> None:
        """Log workspace update event"""
        self._log_event(
            AuditEventType.WORKSPACE_UPDATED,
            {"workspace_id": workspace_id, "changes": changes},
        )
    ...
    def log_item_update(
        self, workspace_id: str, item_id: str, item_name: str, changes: Dict[str, Any]
    ) -> None:
    ...
    def log_git_update(
        self, workspace_id: str, conflict_resolution: str, items_updated: int
    ) -> None:
    ...
    def log_user_removal(
        self, workspace_id: str, user_email: str, reason: Optional[str] = None
    ) -> None:
        """Log user removal event"""
        self._log_event(
            AuditEventType.USER_REMOVED,
            {"workspace_id": workspace_id, "user_email": user_email, "reason": reason},
        )

    def log_user_role_change(
        self, workspace_id: str, user_email: str, old_role: str, new_role: str
    ) -> None:
    ...
    def log_validation_failure(
        self, item_name: str, item_type: str, validation_errors: List[str]
    ) -> None:
    ...
    def log_validation_success(self, item_name: str, item_type: str) -> None:
    ...
    def generate_compliance_report(
        ...
        workspaces_created = sum(
            1 for e in events if e.get("event_type") == AuditEventType.WORKSPACE_CREATED
        )

        # Count items created
        items_created = sum(
            1 for e in events if e.get("event_type") == AuditEventType.ITEM_CREATED
        )

        # Count Git operations
        git_connections = sum(
            1 for e in events if e.get("event_type") == AuditEventType.GIT_CONNECTED
        )

        # Count validation failures
        validation_failures = sum(
            1 for e in events if e.get("event_type") == AuditEventType.VALIDATION_FAILED
        )
