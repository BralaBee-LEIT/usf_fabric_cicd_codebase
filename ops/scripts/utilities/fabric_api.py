...
class FabricClient:
    ...
    def list_workspace_items(self, workspace_id: str, item_type: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def create_or_update_notebook(self, workspace_name: str, notebook_name: str, content_bytes: bytes) -> Dict[str, Any]:
        ...
        existing_notebook = next((item for item in existing_items if item['displayName'] == notebook_name), None)
        ...
        try:
            notebook_content = json.loads(content_str) if isinstance(content_str, str) else content_str
        ...
        if existing_notebook:
            ...
            logger.info(f"Updated notebook '{notebook_name}' in workspace '{workspace_name}'")
        else:
            # Create new notebook
            endpoint = f"workspaces/{workspace_id}/items"
            response = self._make_request('POST', endpoint, json=payload)
            logger.info(f"Created notebook '{notebook_name}' in workspace '{workspace_name}'")
        
        return response.json()

    def deploy_pipeline_json(self, workspace_name: str, pipeline_json: str) -> Dict[str, Any]:
        """Deploy data pipeline from JSON definition"""
        workspace_id = self.get_workspace_id(workspace_name)
        
        try:
            pipeline_def = json.loads(pipeline_json) if isinstance(pipeline_json, str) else pipeline_json
        ...
        existing_pipeline = next((item for item in existing_items if item['displayName'] == pipeline_name), None)
        ...
        if existing_pipeline:
            ...
            response = self._make_request('PATCH', endpoint, json=payload)
            logger.info(f"Updated pipeline '{pipeline_name}' in workspace '{workspace_name}'")
        else:
            endpoint = f"workspaces/{workspace_id}/items"
            response = self._make_request('POST', endpoint, json=payload)
            logger.info(f"Created pipeline '{pipeline_name}' in workspace '{workspace_name}'")
        
        return response.json()

    def deploy_dataflow(self, workspace_name: str, dataflow_name: str, dataflow_definition: Dict[str, Any]) -> Dict[str, Any]:
        """Deploy Dataflow Gen2"""
        workspace_id = self.get_workspace_id(workspace_name)
        
        existing_items = self.list_workspace_items(workspace_id, "Dataflow")
        existing_dataflow = next((item for item in existing_items if item['displayName'] == dataflow_name), None)
                ...
                "parts": [
                    {
                        ...
                        "payload": base64.b64encode(json.dumps(dataflow_definition).encode()).decode(),
                        "payloadType": "InlineBase64"
                    }
                ]
            }
        ...
        if existing_dataflow:
            ...
            response = self._make_request('PATCH', endpoint, json=payload)
            logger.info(f"Updated dataflow '{dataflow_name}' in workspace '{workspace_name}'")
        else:
            endpoint = f"workspaces/{workspace_id}/items"
            response = self._make_request('POST', endpoint, json=payload)
            logger.info(f"Created dataflow '{dataflow_name}' in workspace '{workspace_name}'")
        
        return response.json()

    def trigger_deployment_pipeline(self, pipeline_id: str, source_stage_id: str, target_stage_id: str, 
        ...
        response = self._make_request('POST', endpoint, json=payload)
        logger.info(f"Triggered deployment from stage {source_stage_id} to {target_stage_id}")
        return response.json()
...
def create_or_update_notebook(workspace: str, name: str, content_bytes: bytes) -> Dict[str, Any]:
