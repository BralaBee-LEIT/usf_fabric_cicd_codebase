...
class EnvironmentConfigManager:
    ...
    def __init__(self, environment: str, config_path: Optional[str] = None):
        ...
        if self.config_manager and not self.config_manager.validate_environment(
            self.environment
        ):
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        if self.config_manager:
            ...
            return {
                "workspace": {
                    "name": self.config_manager.generate_name(
                        "workspace", self.environment
                    ),
                    "capacity": "Premium_P1" if self.environment == "prod" else "Trial",
                },
                "data_sources": {
                    "lakehouse_name": self.config_manager.generate_name(
                        "lakehouse", self.environment
                    ),
                    "storage_account": self.config_manager.generate_name(
                        "storage_account", self.environment
                    ),
                    "key_vault": self.config_manager.generate_name(
                        "key_vault", self.environment
                    ),
                    "data_gateway": self.config_manager.generate_name(
                        "data_gateway", self.environment
                    ),
                },
                "connection_strings": {
                    "sql_server": f"Server={self.config_manager.generate_name('sql_server', self.environment)}",
                    "cosmos_db": self.config_manager.generate_name(
                        "cosmos_db", self.environment
                    ),
                    "event_hub": self.config_manager.generate_name(
                        "event_hub", self.environment
                    ),
                    "service_bus": self.config_manager.generate_name(
                        "service_bus", self.environment
                    ),
                },
                "deployment": {
                    "mode": "standard" if self.environment != "prod" else "promote",
                    "validation_level": (
                        "strict" if self.environment == "prod" else "standard"
                    ),
                    "approval_required": env_config.get(
                        "requires_approval", self.environment in ["test", "prod"]
                    ),
                    "auto_deploy": env_config.get(
                        "auto_deploy", self.environment == "dev"
                    ),
                },
                "monitoring": {
                    "health_check_interval": (
                        "4h" if self.environment == "prod" else "8h"
                    ),
                    "alert_threshold": {
                        "response_time_ms": (
                            2000 if self.environment == "prod" else 5000
                        ),
    ...
    def substitute_parameters(self, content: str) -> str:
        ...
        substitutions = {
            ...
            r"\$\{STORAGE_ACCOUNT\}": self.env_config["data_sources"][
                "storage_account"
            ],
    ...
    def substitute_pipeline_parameters(
        self, pipeline_json: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def validate_environment_config(self) -> Dict[str, Any]:
        ...
        if not re.match(r"^[a-zA-Z0-9-_]+$", workspace_name):
            validation_results["warnings"].append(
                f"Workspace name '{workspace_name}' contains invalid characters"
            )
