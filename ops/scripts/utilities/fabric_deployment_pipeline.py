...
class FabricDeploymentManager:
    ...
    def list_pipeline_stages(self, pipeline_id: str) -> List[Dict[str, Any]]:
        ...
        response = self.fabric_client._make_request('GET', f'pipelines/{pipeline_id}/stages')
        return response.json().get('value', [])
    
    def get_stage_artifacts(self, pipeline_id: str, stage_id: str) -> List[Dict[str, Any]]:
        """Get artifacts in a specific pipeline stage"""
        response = self.fabric_client._make_request('GET', f'pipelines/{pipeline_id}/stages/{stage_id}/artifacts')
        return response.json().get('value', [])
    
    def promote_to_next_stage(self, pipeline_id: str, source_stage_order: int, target_stage_order: int, 
                            items: Optional[List[str]] = None, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Promoting from stage {source_stage_order} to stage {target_stage_order}")
        logger.debug(f"Promotion payload: {json.dumps(payload, indent=2)}")
        
        response = self.fabric_client._make_request('POST', f'pipelines/{pipeline_id}/deployments', json=payload)
        ...
        if 'id' in deployment_result:
            ...
            final_status = self._wait_for_deployment_completion(pipeline_id, deployment_id)
            deployment_result['final_status'] = final_status
        
        return deployment_result
    
    def _wait_for_deployment_completion(self, pipeline_id: str, deployment_id: str, 
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                response = self.fabric_client._make_request('GET', f'pipelines/{pipeline_id}/deployments/{deployment_id}')
                status = response.json()
                
                deployment_status = status.get('status', '').upper()
                
                if deployment_status in ['SUCCEEDED', 'FAILED', 'CANCELLED']:
                    logger.info(f"Deployment {deployment_id} completed with status: {deployment_status}")
                    return status
                
                logger.info(f"Deployment {deployment_id} status: {deployment_status}, waiting {DEFAULT_POLLING_INTERVAL_SECONDS}s...")
        ...
        return {"status": "TIMEOUT", "message": f"Deployment timed out after {timeout_minutes} minutes"}
    
    def get_deployment_history(self, pipeline_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent deployment history"""
        response = self.fabric_client._make_request('GET', f'pipelines/{pipeline_id}/deployments?$top={limit}')
        return response.json().get('value', [])
    
    def create_deployment_pipeline(self, display_name: str, description: str = "") -> Dict[str, Any]:
    ...
    def add_workspace_to_stage(self, pipeline_id: str, stage_order: int, workspace_id: str) -> Dict[str, Any]:
        """Assign workspace to deployment pipeline stage"""
        payload = {
            "workspaceId": workspace_id
        }
        
        response = self.fabric_client._make_request('POST', f'pipelines/{pipeline_id}/stages/{stage_order}/assignWorkspace', json=payload)
        logger.info(f"Assigned workspace {workspace_id} to stage {stage_order}")
        return response.json()
    
    def validate_deployment_readiness(self, pipeline_id: str, source_stage_order: int, 
                                    target_stage_order: int) -> Dict[str, Any]:
        ...
        source_stage = next((s for s in stages if s['order'] == source_stage_order), None)
        target_stage = next((s for s in stages if s['order'] == target_stage_order), None)
        ...
        validation_result = {
            ...
            "source_artifacts": [artifact['displayName'] for artifact in source_artifacts[:5]]  # First 5
        }
...
class FabricGitIntegration:
    ...
    def connect_to_git(self, git_provider: str, organization: str, repository: str, 
                      branch: str, directory_path: str = "/") -> Dict[str, Any]:
        ...
        response = self.fabric_client._make_request(
            'POST', 
            f'workspaces/{self.workspace_id}/git/connect',
            json=payload
        )
        
        logger.info(f"Connected workspace {self.workspace_name} to {git_provider}:{organization}/{repository}#{branch}")
        return response.json()
    
    def sync_to_git(self, commit_message: str, changes: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        response = self.fabric_client._make_request(
            'POST',
            f'workspaces/{self.workspace_id}/git/commitToGit', 
            json=payload
        )
    ...
    def sync_from_git(self) -> Dict[str, Any]:
        ...
        response = self.fabric_client._make_request(
            'POST',
            f'workspaces/{self.workspace_id}/git/updateFromGit',
            json=payload
        )
    ...
    def get_git_status(self) -> Dict[str, Any]:
        ...
        response = self.fabric_client._make_request('GET', f'workspaces/{self.workspace_id}/git/status')
        return response.json()
    
    def disconnect_from_git(self) -> Dict[str, Any]:
        """Disconnect workspace from Git"""
        response = self.fabric_client._make_request('POST', f'workspaces/{self.workspace_id}/git/disconnect')
