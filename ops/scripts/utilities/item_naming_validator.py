...
class ItemNamingValidator:
    ...
    def __init__(self, standards_file: Optional[Path] = None):
        ...
        self.warn_on_deviation = self.standards.get('validation', {}).get('warn_on_deviation', True)
        self.auto_fix_enabled = self.standards.get('validation', {}).get('auto_fix_suggestions', True)
    ...
    def _validate_ticket_naming(
        ...
        if ticket_prefix not in valid_systems:
            warnings = [f"Ticket system '{ticket_prefix}' not in recognized systems: {valid_systems}"]
    ...
    def _suggest_fix(
        ...
        if item_type == "Lakehouse":
            ...
            descriptive = '_'.join([self._to_camel_case(p) for p in remaining_parts if p])
