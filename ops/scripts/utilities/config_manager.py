        ...
        if environment and environment in self.config.get("environments", {}):
            ...
            for resource_type in self.config.get("naming_patterns", {}):
                try:
                    ...
                    logger.debug(
                        f"Failed to generate name for {resource_type}: {type(e).__name__} - {e}"
                    )
                    continue
