...
class NotebookOutputChecker:
        ...
        try:
            ...
            if has_outputs:
                ...
                if auto_clear:
                    ...
                    logger.warning(f"Found outputs in {notebook_path.name} ({cell_outputs_count} outputs)")
            else:
    ...
    def generate_report(self, format: str = 'text') -> str:
        ...
        lines.append(f"Notebooks with outputs: {self.results['notebooks_with_outputs']}")
        ...
        if self.results['notebooks_with_outputs'] > 0:
            ...
            for notebook in self.results['details']:
                if notebook['has_outputs']:
                    ...
                    lines.append(f"- {notebook['path']} - {status} ({notebook['cell_outputs_count']} outputs)")
            lines.append("")
...
def main():
    ...
    parser.add_argument("--fail-on-outputs", action="store_true",
    ...
    try:
        ...
        if args.fail_on_outputs and checker.has_violations():
            if not args.clear:
                logger.error(f"Found {results['notebooks_with_outputs']} notebooks with outputs")
                return 1
        
        if results['errors']:
            logger.error(f"Encountered {len(results['errors'])} errors during processing")
            return 1
