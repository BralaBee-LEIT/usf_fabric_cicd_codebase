...
class FabricDeployer:
    ...
    def _deploy_notebook(self, filename: str, content: bytes):
        ...
        self._track_deployment(notebook_name, 'notebook', operation, previous_state=None)
        
        result = fabric_client.create_or_update_notebook(self.workspace, notebook_name, content)
    ...
    def _deploy_pipeline(self, filename: str, content: bytes):
        ...
        try:
            ...
            pipeline_def = self.config_manager.substitute_pipeline_parameters(pipeline_def)
            pipeline_json_str = json.dumps(pipeline_def)
        except json.JSONDecodeError as e:
            logger.warning(f"Could not parse pipeline JSON for parameter substitution: {e}")
        
        result = fabric_client.deploy_pipeline_json(self.workspace, pipeline_json_str)
        logger.info(f"Successfully deployed pipeline: {filename} with environment config: {self.environment}")
    ...
    def _deploy_dataflow(self, filename: str, content: bytes):
        ...
        result = fabric_client.deploy_dataflow(self.workspace, dataflow_name, dataflow_def)
    ...
    def _generate_deployment_report(self) -> Dict[str, Any]:
        ...
        total_deployed = sum(stats['deployed'] for stats in self.deployment_stats.values())
        ...
        report = {
            ...
            'summary': {
                ...
                'success_rate': f"{(total_deployed/(total_deployed+total_failed)*100):.1f}%" if (total_deployed+total_failed) > 0 else "0%"
            },
            'details': self.deployment_stats,
            'deployment_history': self.deployment_history,
            'status': 'SUCCESS' if total_failed == 0 else 'PARTIAL_SUCCESS' if total_deployed > 0 else 'FAILED'
        }
        
        logger.info(f"Deployment Report: {json.dumps(report, indent=2)}")
        return report
    
    def _track_deployment(self, artifact_name: str, artifact_type: str, operation: str, previous_state: Any = None):
        ...
        deployment_record = {
            ...
            'timestamp': json.dumps({'__timestamp__': True}),  # Placeholder for actual timestamp
            'workspace': self.workspace
        }
    ...
    def rollback_deployment(self) -> Dict[str, Any]:
        ...
        logger.info(f"Starting rollback of {len(self.deployment_history)} operations...")
        rollback_actions = []
        ...
        return {
            ...
            'total_rolled_back': len([a for a in rollback_actions if a['status'] == 'SUCCESS'])
        }
    ...
    def _rollback_single_artifact(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation == 'create':
            ...
            if previous_state:
                ...
                return {'artifact': artifact_name, 'action': 'restored', 'status': 'SUCCESS'}
            else:
                logger.warning(f"No previous state available for: {artifact_name}")
                return {'artifact': artifact_name, 'action': 'skipped', 'status': 'NO_PREVIOUS_STATE'}
            ...
            if previous_state:
                ...
                return {'artifact': artifact_name, 'action': 'recreated', 'status': 'SUCCESS'}
            else:
...
def main():
    ...
    parser.add_argument("--git-repo", help="Path to Git repository for direct deployment")
    parser.add_argument("--mode", default="standard", choices=["standard", "promote", "validation"], 
                       help="Deployment mode")
    parser.add_argument("--validate-only", action="store_true", 
                       help="Only validate artifacts without deploying")
    parser.add_argument("--deployment-pipeline-id", help="Fabric deployment pipeline ID for promotion")
    ...
    try:
        ...
        if args.validate_only:
            logger.info("Validation-only mode - no actual deployments will be performed")
