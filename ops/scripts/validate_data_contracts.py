...
class DataContractValidator:
    ...
    def discover_contracts(self) -> List[Path]:
        ...
        if not self.contracts_dir.exists():
            logger.warning(f"Data contracts directory {self.contracts_dir} does not exist")
            return []
    ...
    def _validate_schema_definition(self, schema: Any) -> List[Dict[str, Any]]:
        ...
        for i, field in enumerate(schema):
            ...
            if "type" not in field:
                ...
                # Validate data type
                valid_types = ["string", "integer", "float", "boolean", "timestamp", "date", "decimal", "binary"]
    ...
    def _validate_contract_completeness(self, contract_data: Dict[str, Any], warnings: List[Dict[str, Any]]):
        ...
        for result in results["results"]:
            ...
            if result["issues"]:
                ...
                for issue in result["issues"]:
                    ...
                    lines.append(f"- {severity_icon} **{issue.get('type', 'unknown')}**: {issue.get('message', 'No message')}")
                lines.append("")
            
            if result["warnings"]:
                lines.append("### Warnings:")
                for warning in result["warnings"]:
                    lines.append(f"- ⚠️ **{warning.get('type', 'warning')}**: {warning.get('message', 'No message')}")
                lines.append("")
...
def main():
    ...
    parser.add_argument("--contracts-dir", default="governance/data_contracts", 
