...
class FabricGitSyncManager:
    ...
    def __init__(self, workspace_name: str):
        ...
        self.sync_stats = {"items_synced": 0, "errors": 0, "warnings": 0}
    ...
    def sync_to_workspace(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Current Git status: {git_status.get('gitSyncStatus', 'Unknown')}"
            )
            ...
            logger.info(
                f"Successfully synced {self.sync_stats['items_synced']} items to workspace"
            )
    ...
    def sync_to_git(self, commit_message: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully synced {self.sync_stats['items_synced']} items to Git"
            )
    ...
    def initialize_git_connection(
        ...
        try:
            ...
            logger.info(
                f"Successfully connected workspace to {git_provider}:{organization}/{repository}#{branch}"
            )
            return {
                "status": "success",
                "workspace": self.workspace_name,
...
def main():
    ...
    parser.add_argument(
        "--commit-message", help="Commit message for workspace-to-git sync"
    )
    parser.add_argument(
        "--git-provider",
    ...
    parser.add_argument(
        "--organization", help="Git organization/owner (required for init-git)"
    )
    parser.add_argument(
        "--repository", help="Git repository name (required for init-git)"
    )
    parser.add_argument("--branch", default="main", help="Git branch (default: main)")
    parser.add_argument(
        "--directory", default="/", help="Directory path in repository (default: /)"
    )
    parser.add_argument(
        "--output-format",
        choices=["json", "text"],
    ...
    if args.action == "init-git":
        if not all([args.git_provider, args.organization, args.repository]):
            parser.error(
                "init-git action requires --git-provider, --organization, and --repository"
            )
        ...
        if args.output_format == "json":
            ...
            if result.get("status") == "success":
                ...
                if "git_status" in result:
                    ...
                    print(
                        f"Git sync status: {git_status.get('gitSyncStatus', 'Unknown')}"
                    )
                    if git_status.get("gitProviderDetails"):
                        provider_details = git_status["gitProviderDetails"]
                        print(
                            f"Connected to: {provider_details.get('gitProviderType')} - {provider_details.get('repositoryName')}"
                        )
