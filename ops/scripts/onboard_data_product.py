...
def git_branch_exists(repo_path: Path, branch_name: str) -> bool:
    ...
    code, _, _ = run_command(
        ["git", "rev-parse", "--verify", branch_name], cwd=repo_path
    )
    return code == 0
...
def git_checkout_new(repo_path: Path, branch_name: str, base_branch: str) -> None:
    ...
    if code != 0:
        raise RuntimeError(
            stderr or f"Failed to create branch {branch_name} from {base_branch}"
        )
...
def git_stage_and_commit(repo_path: Path, paths: List[Path], message: str) -> None:
    ...
    if code != 0:
        ...
        status_code, status_out, _ = run_command(
            ["git", "status", "--porcelain", *rel_paths], cwd=repo_path
        )
...
class DataProductOnboarder:
    def __init__(self, arguments: OnboardingArguments):
        ...
        self.templates_dir = (
            self.repo_root / "data_products" / "templates" / "base_product"
        )
        ...
        if not self.credentials_available and not (
            self.args.skip_workspaces or self.dry_run
        ):
            console_warning(
    ...
    def load_descriptor(self) -> ProductDescriptor:
        ...
        product = ProductDescriptor(
            ...
            audit_reference=descriptor_data.get("automation", {}).get(
                "audit_reference"
            ),
            git=git_config,
            dev=dev_descriptor,
        ...
        if (
            ...
            if git_config.get("enabled") and git_config.get("auto_connect_workspaces"):
                try:
                    ...
                    console_info(
                        f"Connecting workspace to Git: {product.git.organization}/{product.git.repository}#{branch_name}"
                    )
                    self.git_connector.initialize_git_connection(
                        ...
                        or git_config.get("organization"),
                        project_name=git_config.get("project"),
                        repository_name=product.git.repository
                        or git_config.get("repository"),
                        branch_name=branch_name,
                        directory_path=directory_path,
                    )
                    console_success(
                        f"Connected DEV workspace to Git branch '{branch_name}'"
                    )
    ...
    def update_registry(
        self, product: ProductDescriptor, result: OnboardingResult
    ) -> None:
        if self.dry_run:
        ...
        if self.registry_path.exists():
            with self.registry_path.open("r", encoding="utf-8") as handle:
                try:
                    ...
                    console_warning(
                        "Registry file is corrupted. Overwriting with new data."
                    )
        ...
        if result.feature_workspace and self.args.feature_ticket:
            entry["feature_workspaces"][self.args.feature_ticket] = (
                result.feature_workspace.get("id")
            )
    ...
    def write_audit_log(self, result: OnboardingResult) -> Path:
        ...
        payload = {
            ...
            "scaffold_path": (
                str(result.scaffold_path) if result.scaffold_path else None
            ),
    ...
    def ensure_git_branch(
        ...
        repo = self.repo_root
        branch_name = (
            f"{product.slug}/{product.git.feature_prefix}/{self.args.feature_ticket}"
        ...
        if git_branch_exists(repo, branch_name):
            ...
            if product.git.auto_commit and result.scaffold_path:
                console_info(
                    f"Updating scaffold commit on existing branch {branch_name}"
                )
        ...
        try:
            ...
            if product.git.auto_commit and result.scaffold_path:
                commit_message = f"chore(onboarding): bootstrap {product.name} ({self.args.feature_ticket})"
    ...
    def connect_feature_workspace_to_git(
        ...
        if not (product.git.organization and product.git.repository):
            console_warning(
                "Git organization/repository not provided; cannot connect workspace to Git"
            )
            return

        # Skip automated Git connection if manual integration is preferred
        skip_auto_connect = (
            os.getenv("SKIP_AUTO_GIT_CONNECT", "false").lower() == "true"
        )
        ...
        if self.git_connector:
            try:
                directory_path = (
                    f"/data_products/{product.slug}/{self.args.feature_ticket}"
                )

                console_info(
                    f"Connecting feature workspace to Git: {product.git.organization}/{product.git.repository}#{branch_name}"
                )
                ...
                console_success(
                    f"Connected feature workspace '{workspace_name}' to Git branch '{branch_name}'"
                )
                ...
                console_warning(
                    f"Git connector failed, falling back to legacy method: {exc}"
                )
    ...
    def run(self) -> OnboardingResult:
        ...
        console_info(
            f"Starting onboarding for product '{product.name}' (slug: {product.slug})"
        )
        ...
        if self.args.feature_ticket:
            feature_workspace, feature_created = self.ensure_feature_workspace(
                product, self.args.feature_ticket
            )
        ...
        if self.audit_logger and not self.dry_run:
            self.audit_logger.log_onboarding_completed(
                product_id=product.slug,
                workspace_id=(
                    result.dev_workspace.get("id") if result.dev_workspace else None
                ),
                feature_workspace_id=(
                    result.feature_workspace.get("id")
                    if result.feature_workspace
                    else None
                ),
                git_branch=result.git_branch,
            )
