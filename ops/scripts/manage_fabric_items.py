    ...
    try:
        ...
        if args.json:
            print_json([{
                ...
                'createdDate': item.created_date.isoformat() if item.created_date else None,
                'modifiedDate': item.modified_date.isoformat() if item.modified_date else None
            } for item in items])
        else:
            if not items:
            ...
            for item in items:
                rows.append([
                    ...
                    item.created_date.strftime('%Y-%m-%d') if item.created_date else 'N/A'
                ])
    ...
    try:
        ...
        if args.json:
            ...
            if item.created_date:
                print_info(f"  Created: {item.created_date.strftime('%Y-%m-%d %H:%M:%S')}")
            if item.modified_date:
                print_info(f"  Modified: {item.modified_date.strftime('%Y-%m-%d %H:%M:%S')}")
...
def cmd_delete_item(args):
    ...
    try:
        ...
        # Confirm deletion
        if not args.force:
            response = input(f"Are you sure you want to delete '{item_name}'? Type 'DELETE' to confirm: ")
            if response != 'DELETE':
...
def cmd_bulk_delete(args):
    ...
    try:
        ...
        # Confirm deletion
        if not args.force:
            response = input(f"Delete {len(item_ids)} item(s)? Type 'DELETE {len(item_ids)}' to confirm: ")
...
Examples:
  ...
  %(prog)s bulk-delete --workspace dev-workspace --file item_ids.txt
    ...
    create_parser.add_argument('--definition-file', help='JSON file with item definition')
    ...
    bulkdel_parser.add_argument('--force', action='store_true', help='Skip confirmation')
