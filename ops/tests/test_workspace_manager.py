...
class TestWorkspaceOperations:
    ...
    def test_create_workspace(self, mock_token, mock_request, workspace_manager, mock_workspace_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_create_workspace_duplicate_error(self, mock_token, mock_request, workspace_manager):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_list_workspaces(self, mock_token, mock_request, workspace_manager, mock_workspace_response):
        """Test listing workspaces"""
        mock_token.return_value = 'test-token'
        mock_response = Mock()
        mock_response.json.return_value = {
            'value': [mock_workspace_response, {**mock_workspace_response, 'id': 'workspace-456'}]
        }
        mock_request.return_value = mock_response
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_list_workspaces_filtered_by_environment(self, mock_token, mock_request, workspace_manager):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_get_workspace_details(self, mock_token, mock_request, workspace_manager, mock_workspace_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager.get_workspace_details')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager.list_workspace_items')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_delete_workspace(self, mock_token, mock_items, mock_details, mock_request, workspace_manager, mock_workspace_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager.list_workspace_items')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_delete_workspace_with_items_error(self, mock_token, mock_items, mock_details, workspace_manager, mock_workspace_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_update_workspace(self, mock_token, mock_request, workspace_manager, mock_workspace_response):
        """Test updating workspace"""
        mock_token.return_value = 'test-token'
        mock_response = Mock()
        mock_response.json.return_value = {**mock_workspace_response, 'displayName': 'updated-workspace-dev'}
        mock_request.return_value = mock_response
...
class TestUserManagement:
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_add_user(self, mock_token, mock_request, workspace_manager, mock_user_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_add_user_duplicate_error(self, mock_token, mock_request, workspace_manager):
    ...
    def test_remove_user(self, mock_token, mock_request, workspace_manager):
        ...
        mock_request.assert_called_once_with(
            'DELETE',
            'v1/workspaces/workspace-123/roleAssignments/user@example.com'
        )
    
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_list_users(self, mock_token, mock_request, workspace_manager, mock_user_response):
        ...
        mock_response.json.return_value = {
            'value': [
                mock_user_response,
                {**mock_user_response, 'identifier': 'user2@example.com', 'workspaceRole': 'Member'}
            ]
        }
        mock_request.return_value = mock_response
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._make_request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_update_user_role(self, mock_token, mock_request, workspace_manager, mock_user_response):
        """Test updating user role"""
        mock_token.return_value = 'test-token'
        mock_response = Mock()
        mock_response.json.return_value = {**mock_user_response, 'workspaceRole': 'Contributor'}
        mock_request.return_value = mock_response
...
class TestBulkOperations:
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_create_workspace_set(self, mock_token, mock_create, workspace_manager, mock_workspace_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_copy_users_between_workspaces(self, mock_token, mock_add, mock_list, workspace_manager):
        """Test copying users between workspaces"""
        mock_token.return_value = 'test-token'
        mock_list.return_value = [
            {'identifier': 'user1@example.com', 'principalType': 'User', 'workspaceRole': 'Admin'},
            {'identifier': 'user2@example.com', 'principalType': 'User', 'workspaceRole': 'Member'}
        ]
...
class TestErrorHandling:
    ...
    @patch('ops.scripts.utilities.workspace_manager.requests.request')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager._get_access_token')
    def test_retry_on_transient_error(self, mock_token, mock_request, workspace_manager):
...
class TestConvenienceFunctions:
    ...
    def test_create_workspace_for_environment(self, mock_config, mock_create, mock_env_vars, mock_workspace_response):
    ...
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager.create_workspace_set')
    @patch('ops.scripts.utilities.workspace_manager.WorkspaceManager.add_user')
    @patch('ops.scripts.utilities.workspace_manager.get_config_manager')
    def test_setup_complete_environment(self, mock_config, mock_add, mock_create_set, mock_env_vars):
