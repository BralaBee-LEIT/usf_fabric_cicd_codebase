...
> **Version**: 1.8.3 | **Last validated**: 16 March 2026
>
...
# Inline environment overrides (v1.8.3+)
environments:
  dev:
    workspace:
      name: ${PROJECT_PREFIX}
...
- **`environments:`** block (v1.8.3+) allows inline per-environment overrides — these take priority over external files in `config/environments/`
    ...
    steps:
      ...
      - name: Install CLI
        run: |
          ...
          pip install "git+https://${{ secrets.FABRIC_GITHUB_TOKEN }}@${{ vars.CLI_REPO_URL || 'github.com/your-org/your-cli-repo' }}.git@${{ vars.CLI_REPO_REF || 'v1.8.3' }}"
