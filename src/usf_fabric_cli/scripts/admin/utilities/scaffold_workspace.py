...
def _generate_yaml(
    ...
    if templatise:
        ...
        lines.append(
            "  # -- Mandatory governance principals"
            " (resolved via GitHub Secrets) --"
        )
        lines.append('  - id: "${AZURE_CLIENT_ID}"')
        lines.append("    type: ServicePrincipal")
        lines.append("    role: Admin")
        ...
        lines.append("")
        lines.append(
            "  # -- Discovered principals from live workspace (active) --"
        )
        for p in discovered_principals:
...
def _generate_feature_yaml(
    ...
    lines.append("# Principals -- same as base workspace for full access on feature branches")
    lines.append("principals:")
    lines.append('  - id: "${AZURE_CLIENT_ID}"')
    lines.append("    type: ServicePrincipal")
    lines.append("    role: Admin")
