...
class FabricDatasourceRepointAPI(FabricAPIBase):
    ...
    def list_item_connections(
        ...
        try:
            ...
            logger.warning(
                "Failed to list connections for item %s: %s", item_id, e
            )
            return []
    ...
    def get_lakehouse(
        ...
        try:
            ...
            logger.warning(
                "Failed to get lakehouse %s: %s", lakehouse_id, e
            )
            return None
    ...
    def bind_connection(
        ...
        try:
            ...
            if status == 403:
                ...
                reason = (
                    f"bindConnection API returned HTTP {status}"
    ...
    def rebind_direct_lake_models(
        ...
        for lh in target_lakehouses:
            ...
            if conn_string and endpoint_id:
                target_name_to_path[lh["displayName"]] = (
                    f"{conn_string};{endpoint_id}"
                )
        ...
        for model in models:
            ...
            connections = self.list_item_connections(
                target_workspace_id, model_id
            )
            if not connections:
            ...
            for conn in connections:
                ...
                outcome = self.bind_connection(
                    target_workspace_id, model_id, binding
                )
                if outcome["success"]:
                    result.repointed.append(
                        {
                ...
                result.skipped.append(
                    {
                        "model": model_name,
                        "reason": (
                            "no Direct Lake connections reference source "
                            "workspace"
                        ),
                    }
                )
