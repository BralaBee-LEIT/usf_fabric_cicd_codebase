            ...
            if contracts_dir.exists():
                ...
                for contract_file in contract_files:
                    try:
                        ...
                        if missing:
                            print(
                                f"  ⚠️  {contract_file.name} - Missing fields: {missing}"
                            )
                        else:
                            print(
                                f"  ✅ {contract_file.name} - Basic validation passed"
                            )
            ...
            if rules_dir.exists():
                ...
                for rules_file in rules_files:
                    try:
                        ...
                        # Basic validation
                        if "rules" in rules_data and isinstance(
                            rules_data["rules"], list
                        ):
                            rule_count = len(rules_data["rules"])
                            print(
                                f"  ✅ {rules_file.name} - {rule_count} rule(s) found"
                            )
                        else:
        ...
        try:
            ...
            for job_name, job_config in jobs.items():
                ...
                for step in steps:
                    ...
                    if (
                        "validate" in step_name.lower()
                        or "contract" in step_name.lower()
                    ):
                        validation_found = True
                        print(
                            f"  ✅ Found validation step: '{step_name}' in job '{job_name}'"
                        )
